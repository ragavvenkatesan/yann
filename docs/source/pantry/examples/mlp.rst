...
``pantry.examples.mlp.py``. Run the code as following:
...
.. code-block:: bash
    ...
    python pantry/examples/mlp.py 

from the toolbox root or path added to toolbox. The ``__init__`` program has all the required
tools to create or load a dataset created. Optionally as command line argument you can provide 
the location to the dataset.

.. automodule:: pantry.examples.mlp
   :members:
