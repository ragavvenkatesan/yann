...
class split_continual(setup_dataset):
    
    """
    Inheriting from the setup dataset. This method will produce datasets setup for continual 
    learning systems. 
    """    
    def __init__(self,
                 dataset_init_args,
        ...
        """
        This is just a re-use of the setup_dataset. This will construct the split dataset. 
        In the default arguments listed below, this will create data 2,3 classes for training, 
        will test on the classes of 0,1,2,3 and not have classes 4-9 in the dataset.
        ...
        Args:
            ...
            n_classes: supply the number of classes in the dataset. 
        ...
        self.n_classes = len(self.splits["test"])
        
        super(split_continual,self).__init__( 
                                        dataset_init_args = dataset_init_args,
                                        save_directory = save_directory,
                                        preprocess_init_args = self.preprocessor,
                                        verbose = 1)
    ...
    def _create_skdata_mnist(self, verbose = 1):
        ...
        data_x, data_y, data_y1 = self._split_data (data = data[2], keep = self.splits["test"])
        data_x = preprocessing ( data = data_x,
                                 height = self.height,
                                 width = self.width,
                                 channels = self.channels,
    ...
    def _mat2yann (self, verbose = 1):
        ...
        data_x, data_y  = self._split_data (data = ( data_x, data_y ), y1 = False, 
                                                                    keep = self.splits["train"] )
        data_x = preprocessing ( data = data_x,
                                height = self.height,
                                width = self.width,
                                channels = self.channels,
    ...
    def _split_data (self, data, keep, y1 = True):
        """
        This is an internal method that will split the datasets.

        Args:
            data: train, test and valid batches in a tuple.
            keep: which labels to keep. Everythin else is dropped. 
